...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    startLoading();
    await axios.get('http://localhost:8080/api/v1/orders/view-orders', {
      headers :{
    ...
    orders.forEach(order => {
      ...
      productElement.innerHTML = `
        <div class="product">
            ...
            <h3>Order Date: ${order.orderDate}</h4>
            <h3  style="color:#e74c3c;">Order Status: ${order.orderStatus}</h4>
            </div>
            <div class="product-description">
            <h3>State: ${order.state}</h3>
            <h3>Takuk: ${order.takul}</h3>
            <h3>Total Plant Ordered: ${order.totalPlant}</h4>
        </div>
